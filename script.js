...
const translations = {
  ru: {
    ...
    // Hero
    hero_invitation: 'ПРИГЛАШЕНИЕ НА 3-ДНЕВНЫЙ ИНТЕНСИВ',
  ...
  ua: {
    ...
    // Hero
    hero_invitation: 'ЗАПРОШЕННЯ НА 3-ДЕННИЙ ІНТЕНСИВ',
...
// ─────────── TEMPORARY IMAGE ADJUSTMENT TOOL ───────────
document.addEventListener('DOMContentLoaded', () => {
  const panel = document.createElement('div');
  panel.style.cssText = 'position:fixed;bottom:20px;right:20px;background:var(--bg-deep, #1C1C1E);color:#fff;padding:20px;z-index:99999;border-radius:12px;font-family:monospace;box-shadow:0 10px 30px rgba(0,0,0,0.7);width:320px;border:1px solid #c5a880;';
  
  const title = document.createElement('h3');
  title.innerText = '🛠 Настройка позиций фото';
  title.style.cssText = 'margin:0 0 10px 0;font-size:16px;color:#c5a880;font-family:sans-serif;';
  panel.appendChild(title);

  const instr = document.createElement('p');
  instr.innerText = 'Тяните (drag) любую фотографию ВВЕРХ или ВНИЗ мышкой, чтобы настроить её позицию.\n\nКогда закончите, просто скиньте мне эти цифры, и я сохраню их навсегда.';
  instr.style.cssText = 'margin:0 0 15px 0;font-size:13px;line-height:1.4;opacity:0.8;font-family:sans-serif;';
  panel.appendChild(instr);

  const pre = document.createElement('pre');
  pre.style.cssText = 'font-size:12px;background:#000;padding:12px;border-radius:6px;max-height:250px;overflow-y:auto;white-space:pre-wrap;margin:0;color:#0f0;';
  ...
  function updatePanel() {
    let output = '';
    // Only get unique original images
    const uniqueImages = Array.from(document.querySelectorAll('.carousel-slide:not(.clone) img'));
    uniqueImages.forEach(img => {
      const src = img.getAttribute('src').split('/').pop();
      const pos = img.style.objectPosition || 'center 15%';
      output += `${src}: ${pos}\n`;
    });
  ...
  // Wait slightly for carousel clones to be created
  setTimeout(() => {
    document.querySelectorAll('.carousel-slide img').forEach(img => {
      img.style.pointerEvents = 'auto'; // allow dragging on img
      ...
      img.addEventListener('mousedown', (e) => {
        ...
        const currentPos = img.style.objectPosition || window.getComputedStyle(img).objectPosition;
        const match = currentPos.match(/(\d+)%/);
        startPos = match ? parseFloat(match[1]) : 15;
        
        e.stopPropagation(); // stop carousel from sliding left/right
  ...
  window.addEventListener('mousemove', (e) => {
    if (!isAdjusting || !activeImg) return;
    const deltaY = e.clientY - startY;
    let newPos = startPos - (deltaY * 0.4); 
    newPos = Math.max(0, Math.min(100, Math.round(newPos)));
    
    const src = activeImg.getAttribute('src');
    document.querySelectorAll(`img[src="${src}"]`).forEach(cloneImg => {
      cloneImg.style.objectPosition = `center ${newPos}%`;
    });
    updatePanel();
  });
  ...
  window.addEventListener('mouseup', () => { isAdjusting = false; activeImg = null; });
  window.addEventListener('touchend', () => { isAdjusting = false; activeImg = null; });
});
